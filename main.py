...
class ConnectionManager:
    ...
    async def broadcast(self, message: bytes):
        ...
        await asyncio.gather(*[send_safe(ws) for ws in connections], return_exceptions=True)
...
def get_full_state_bytes() -> bytes:
...
var ws,ra=0;function conn(){var pr=location.protocol==="https:"?"wss:":"ws:";ws=new WebSocket(pr+"//"+location.host+"/ws");ws.binaryType='arraybuffer';ws.onopen=function(){ra=0};ws.onmessage=function(e){try{var d;if(e.data instanceof ArrayBuffer){d=JSON.parse(new TextDecoder().decode(e.data))}else{d=JSON.parse(e.data)}if(d.ping)return;if(d.history)updateTable(d.history);if(d.usd_idr_history)updateUsd(d.usd_idr_history);if(d.treasury_info!==undefined)updateInfo(d.treasury_info)}catch(x){}};ws.onclose=function(){ra++;setTimeout(conn,Math.min(1000*Math.pow(1.5,ra-1),30000))};ws.onerror=function(){}}conn();
...
async def websocket_endpoint(ws: WebSocket):
    ...
    if not await manager.connect(ws):
        ...
        return
    try:
        initial_data = get_full_state_bytes()
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000, log_level="warning", access_log=False, ws_ping_interval=20, ws_ping_timeout=20, limit_concurrency=1000)
